...
Planeamento pessoal de trabalho alimentado por Issues do GitLab e reuniões do Outlook/Teams.

## Funcionalidades

- sincronização paginada de projetos e Issues GitLab atribuídas ao utilizador;
...
Requisitos: Node.js 22 ou superior e Outlook clássico instalado/configurado.

```powershell
git clone https://github.com/nunomoreira1993/work-organizer.git
cd work-organizer
npm install
npm run dev:windows
```
...
```powershell
npm run outlook:bridge
```

Mantém a janela aberta. Depois abre **Definições → Outlook / Teams** e seleciona **Importar do Outlook instalado**. A ponte:

- escuta apenas em `127.0.0.1:47831`;
- aceita apenas o Work Organizer local ou o Site publicado como origem;
- lê assunto, início, fim e link Teams das reuniões dos próximos 35 dias;
- não pede nem guarda credenciais Microsoft.

Após importar, associa cada reunião ao respetivo projeto GitLab para ocupar capacidade na Timeline.

## Segurança

- Nunca coloques tokens GitLab no repositório ou em Issues.
- `.env*`, certificados e artefactos locais estão ignorados.
- A ponte local expõe apenas leitura do calendário futuro e termina quando fechas o processo.

## Arquitetura

Aplicação React/Next.js sobre Vinext, preparada para execução local e publicação em ChatGPT Sites.

---

## Base técnica do Sites
